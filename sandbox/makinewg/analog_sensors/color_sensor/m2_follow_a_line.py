...
def follow_the_line(robot, white_level, black_level):
    ...
    while not robot.touch_sensor.is_pressed:
        if robot.color_sensor.reflected_light_intensity < white_level - 20:
            robot.turn_degrees(-10, 400)
            ev3.Leds.set_color(ev3.Leds.LEFT, ev3.Leds.YELLOW)
            ev3.Leds.set_color(ev3.Leds.RIGHT, ev3.Leds.RED)
        if robot.color_sensor.reflected_light_intensity < 70 and robot.color_sensor.reflected_light_intensity > 60:
            robot.drive(0,0)
